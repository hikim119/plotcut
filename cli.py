...
def cmd_check(a):
    ...
    #
    # **이건 ⚠ 다. 한때 ✘ 였고, 근거가 틀렸다.** 「진 판이 전부 이걸 갖고 있었다」고
    # 적었는데, 진 대본만 들춰 보고 이긴 대본을 안 본 밑수 오류였다. 심사표 82개를
    # 제대로 짝지어 다시 재니(8/26):
    #   · 재인용 수가 갈리는 표 12개 = **독립 대본쌍 3개** (한 쌍당 4표)
    #   · father_v2_dense(재인용 2) 가 base(0) 를 3/4 로 **이겼다**
    #   · father_v1_reaction(2) 는 flow 2승 / gut 2패, gentleman_ship(2) 는 4패
    #   → 셋이 서로 엇갈린다. 승패를 예측하지 못한다.
    # 나레·대사 밀도 지표(셋업 비율·최대 연속 대사·나레 비율)도 같이 쟀는데 전부
    # 47~55% 로 동전 던지기였다 — 심사평의 「셋업 없는 펀치」는 **그 대사에** 셋업이
    # 없다는 말이지 나레가 적다는 말이 아니다. 총량 지표로는 안 잡힌다.
    #
    # 그래도 남긴다: 3분짜리에서 같은 줄을 두 번 쓰는 건 낭비고, 유저 레퍼런스 3편은
    # 0이다. 다만 **증거가 없는 것을 강제로 고치게 하지 않는다** — ⚠ 는 check 루프가
    # 자동으로 손대지 않는다. ✘ 로 올리려면 재인용만 다른 대본쌍으로 승률을 재라.
    _dup = _requoted(doc)
    if _dup:
        notes.append(
            "같은 대사를 두 번 인용한 문단이 %d쌍 있습니다 — 훅으로 앞에 뺀 대사를 "
            "제자리에서 또 쓴 것 같습니다: %s" % (len(_dup), " / ".join(_dup)[:80]))
