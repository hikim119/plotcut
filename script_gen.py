...
def build_prompt(srt_path, out_path, target_s=180, extra="", movie_title="",
    ...
    # 긴 영화에서만 건다. 짧은 영화(시트콤 한 편 등)는 원래 전체를 쓰는 게 맞고,
    # 이 지시를 같이 걸면 과밀해져서 오히려 진다(위 SCENE_MIN_MINUTES 주석).
    if _srt_minutes(srt_path) >= SCENE_MIN_MINUTES:
        lines += [
            "",
            "**영화 전체를 훑지 마라. 한 대목만 써라:**",
            "- 가장 좋은 **연속된 구간 하나**를 골라 그 안에서만 대본을 만든다.",
            "  이 영화 길이면 대략 **10~25분짜리 한 대목** — 영화의 5분의 1 이하다.",
            "- 그 대목 밖의 사건은 아무리 중요해도 버린다. **결말이 그 대목 밖이면",
            "  결말을 쓰지 마라.**",
            "- 블록 수는 그대로 10~12개다. 좁은 구간을 촘촘히 채워라 — 블록끼리",
            "  몇 분씩 건너뛰면 컷이 중구난방이 된다.",
            "- 정답 3편이 실제로 그렇다: 긴 영화 둘은 영화의 13%·21%만 썼고",
            "  둘 다 결말을 안 썼다.",
        ]
    if extra.strip():
...
# 「한 대목만 써라」를 켤 기준. 실측(8/27) — 긴 영화 둘에서 71%(17:7)로 이겼고
# 22분짜리 시트콤에서는 **1:11 로 졌다.** 짧은 영화는 원래 전체를 쓰는 게 맞는데
# 「좁은 구간을 촘촘히」가 같이 걸려서 블록을 줄이고 대사를 밀어 넣었다
# (12~13블록/53~58대사 → 11~12블록/60~64대사). 그래서 **길이로 나눈다.**
SCENE_MIN_MINUTES = 40.0
