...
def _v_flow(lines):
    """나레이션 간격을 **본문 규칙째** 바꾼다 — 덧붙이기로는 안 먹힌다.

    `--extra` 로 「5~8은 잊어라」를 줘 봤지만(flowC, 9판) 간격이 4.1~6.1 로
    오히려 더 빽빽해졌다. 프롬프트 본문과 AGENTS.md 에 「실측: 대사 5~8줄에
    하나」가 실측 권위로 박혀 있어서 그쪽이 이긴다. 그래서 그 줄을 갈아 끼운다.

    근거는 유저분 완성본 **6편 전수**다 — 간격 4.8 · 7.7 · 10.1 · 10.5 · 24.3
    (중앙 10.1)에 `good doctor` 는 나레이션이 **한 줄도 없다**. 더 중요한 건
    고르게 안 뿌린다는 것 — 최대 연속 대사가 12 · 21 · 24 · 35 · 44 · 71 줄이다.
    도구는 간격 6.6 에 최대 연속 13 이라 **6~7줄마다 규칙적으로 끊긴다.**
    """
    out = []
    for ln in lines:
        if "실측: **대사 5~8줄에 하나**" in ln:
            out += [
                "     실측(완성본 6편): **대사 10줄에 하나**가 중앙값이다 —"
                " 4.8 · 7.7 · 10.1 · 10.5 · 24.3,",
                "     한 편은 **나레이션이 한 줄도 없다**(71장 전부 대사).",
                "     그리고 **고르게 뿌리지 마라.** 한 장면 안에서는 대사가 통째로"
                " 이어진다 —",
                "     연속 대사 최대가 12 · 21 · 24 · 35 · 44 · 71 줄이다."
                " 나레이션은 **장면이",
                "     바뀔 때**만 넣어라. 장면 안에서 무슨 일이 벌어지는지 설명하지"
                " 마라.",
            ]
        else:
            out.append(ln)
...
def build_prompt(srt_path, out_path, target_s=180, extra="", movie_title="",
    ...
    lines += [
        ...
        "   · 한 문단 = **자막 큐 하나**가 기본 (실측 94%).",
        "     **문단 안 줄 수는 큐가 아니라 글자 수가 정한다** — 한 줄 8~14자로",
        "     끊어라. 줄 하나가 화면 자막 한 장이고, 나눠도 영상은 안 길어진다.",
        '· 자막 파일: "%s"' % srt_path,
    ...
    if _files:
        ...
        lines += [
            ...
            "   **형식은 따라 하지 마라.** 이건 화면에 뜬 자막이고, 네가 쓸 건 블록",
            "   헤더 + `#번호` 형식이다. 여기서 배우는 건 **문체·리듬과, 무엇을 고르고",
            "   무엇을 버렸는가**다.",
    ...
    if not _srt_is_korean(srt_path):
        lines += [
            ...
            # 예전엔 여기에 「**한 문단 한 줄.** 247큐 중 246이 1줄이다」가 있었다.
            # **인과를 거꾸로 읽은 것이다.** 「246이 1줄」은 완성본의 **화면 카드**가
            # 1줄이라는 뜻이고, 그건 유저분이 긴 대사를 여러 장으로 **쪼갠 결과**다.
            # 그걸 「문단을 1줄로 써라」로 읽는 바람에 대사 장/문단이 정확히 ×1.00 이
            # 됐고(6편 전수, 예외 0건), 카드가 2.2초씩 멈춰 있었다. 실측 비교 —
            #   정답 카드 1.2~1.5초 · 0.65~0.82장/초   도구 2.22초 · 0.49장/초
            # `layout.build` 이 `screen_lines` 의 **줄마다 자막을 하나씩** 내고
            # `total_s` 는 소스 창의 합이라, 줄을 나눠도 **영상 길이는 안 변한다**(실행 확인).
            "   · **한 문단 안에서 줄을 나눠라. 줄 하나가 화면 자막 한 장이 된다.**",
            "     한 줄 8~14자. 완성본 522장의 **중앙이 10자**다.",
            "     긴 대사를 한 줄로 몰면 그 한 장이 2초 넘게 멈춰 있는다 —",
            "     정답은 같은 대사를 두 장으로 나눠 1.2초씩 넘긴다.",
            "     나누는 자리는 **절·문장 경계**다:",
            "       `지옥에 온 걸 환영한다` / `ㅅ끼야`",
            "       `차키랑 지갑 내놔` / `당장!`",
            "       `걱정하지마` / `너한테 줄 게 하나 더 있어`",
    ...
    lines += [
        ...
        "- 정답 4편이 실제로 그렇다: 8.4 · 12.3 · 18.5 · 23.6분짜리 한 대목이고",
        "  긴 영화 둘은 결말을 안 썼다.",
    ]
    if extra.strip():
