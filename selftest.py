...
def variant_tests():
    ...
    # 8/27 **이름 조항을 넣었다 — 이건 대결로 이겨서 들어온 첫 문구다.**
    #   「이 작품을 안다면 그 지식을 이름에 써라」 + 「한 인물은 한 이름으로만」
    #   기준선 대비 24:12(67%) · 세 영화 모두 8:4 · 두 관점 모두 12:6 ·
    ...
    eq("기본 프롬프트가 안 변했다 (한국어 분기)", got, "b304d7a8b4d12016")
...
def requote_tests():
    ...
    재인용은 한때 ✘ 였다. 근거였던 「진 판이 전부 이걸 갖고 있었다」는 진 대본만
    들춰 본 밑수 오류였다 — 심사표 82개를 짝지어 다시 재니 재인용이 갈리는 표는
    독립 대본쌍 **3개**뿐이고 서로 엇갈린다(재인용 2인 father_v2_dense 가 0인
    base 를 3/4 로 이겼다). 승패를 예측하지 못하므로 강제로 고치게 하지 않는다.
    아래 `심각도` 검사가 그 결정을 못 박는다.

    늦은 이름은 「로이스가 결말에서 소개 없이」 — 실측으로 로이스·테런스·코치가
    잡히고 레퍼런스는 0. 처음부터 참고였다.
    """
    import cli
    import script_io as sio
    print()
    ...
    # 심각도 — 재인용은 ⚠(참고)다. ✘ 로 올리면 프롬프트의 check 루프가 강제로
    # 고치게 되는데, 그게 대본을 낫게 한다는 증거가 없다(위 docstring).
    src = pathlib.Path(ROOT / "cli.py").read_text(encoding="utf-8")
    i = src.index("_dup = _requoted(doc)")
    j = src.index("append(", i)
    check("재인용은 강제 수정(✘)이 아니라 참고(⚠)다",
          src[j - 6:j] == "notes.", src[i:j + 8].splitlines()[-1])
