...
# 걸린다 — 원본이 짧으면 한 대목이 전체의 대부분이다. 분으로 재면 정답 최대
# 23.6분 · 도구 49.1분으로 겹침 없이 갈린다.
SPAN_MAX_MIN = 30.0    # 한 대목 모드 상한 — 정답 최대 23.6분 · 도구 49.1분
...
def selection_issues(m, scope="scene"):
    ...
    if scope == "scene":
        if m["span_min"] > SPAN_MAX_MIN:
            warns.append(
                "영화 %.0f분어치를 훑고 있습니다 — 한 대목 모드의 상한은 %.0f분입니다."
                " 완성본 3편 실측 12~24분(영화의 1/8~1/5). 이야기 한 줄기가"
                " 시작해서 닫히는 구간으로 좁히세요."
